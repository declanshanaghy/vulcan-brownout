...
async def main():
    ...
    # Load staging YAML config, fall back to env vars / CLI args
    ...
    # Get HA connection details (CLI args take highest priority)
    ha_url = args.ha_url or cfg.get("HA_URL") or os.getenv("HA_URL", "http://homeassistant.lan:8123")
    ha_token = args.ha_token or cfg.get("HA_TOKEN") or os.getenv("HA_TOKEN")

    if not ha_token:
        logger.error(
            "ERROR: HA_TOKEN not provided. Set it in "
