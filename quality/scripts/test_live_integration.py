...
HA_HOST = os.environ.get("HA_URL", "http://homeassistant.lan")
HA_PORT = os.environ.get("HA_PORT", "8123")
WS_URI = f"ws://{HA_HOST.replace('http://', '').replace('https://', '')}:{HA_PORT}/api/websocket"
REST_URL = f"{HA_HOST}:{HA_PORT}"

if not TOKEN:
    print("ERROR: HA_TOKEN not set. Create .env in project root or export HA_TOKEN.")
    sys.exit(1)
