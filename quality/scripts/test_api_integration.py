...
Usage:
    ...
    pytest quality/scripts/test_api_integration.py -v --capture=no  # Show print statements
"""
...
def ha_connection_params():
    """Load HA connection parameters from environment."""
    env_file = Path(__file__).parent.parent.parent / ".env"
    if env_file.exists():
        from dotenv import load_dotenv
        load_dotenv(env_file)

    ha_url = os.getenv("HA_URL", "http://homeassistant.lan")
    ha_port = os.getenv("HA_PORT", "8123")
    ha_token = os.getenv("HA_TOKEN")

    if not ha_token:
        pytest.skip("HA_TOKEN not set in environment")
...
if __name__ == "__main__":
    ...
    if not ha_token:
        print("ERROR: HA_TOKEN not set in environment")
        sys.exit(1)
